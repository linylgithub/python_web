...
"""
# Author: linyl
# Created Time : Wed Nov 28 17:21:56 2018
# File Name: app_with_mc.py
# Description:
"""
...
app = Flask(__name__)
DATABASE_URI = 'mysql://web:web123@120.79.248.110:3306/r'
api = Api(app)
...
class User(object):
    ...
    def add(cls, name, address):
        sql = """INSERT INTO restful_user(name, address) VALUES(%s, %s)"""
    ...
    @classmethod
    @cache(USER_KEY % '{id_}') #可以添加第二个参数设置缓存时间
    def get(cls, id_):
        if not id_:
            return None
        row = con.execute(
            'select id, name, address from restful_user where id=%s', id_).fetchone()
        return cls(*row) if row else None

    @classmethod
    def get_user_by_name(cls, name):
        sql = "select id from restful_user where name=%s"
        rows = con.execute(sql, name).fetchall()
        return cls.get(*rows[0] if rows else None)
    
    def delete(self):
        con.execute('delete form restful_user where id = %s', self.id)
