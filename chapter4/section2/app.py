...
"""
# Author: linyl
# Created Time : 三 11/21 11:59:52 2018
# File Name: app.py
# Description:
"""
import os 
...
def download(filehash):
    ...
    return send_file(open(paste_file.path, 'rb'),
            ...
            as_attachment=True,
            attachment_filement=paste_file.filename.encode('utf-8'))
...
def preview(filehash):
    ...
    if not paste_file:
        ...
        if not (os.path.exitsts(filepath) and (not os.path.islink(filepath))):
            return abort(404)
        paste_file = PasteFile.create_by_old_paste(filepath)
        db.session.add(paste_file)
        db.session.commit()
